...
struct Weak_LLSC_Thread_Data{
    ...
    std::vector<Buffer*> rList;
    std::vector<Buffer*> fList;
    volatile char padding[192 - 2*sizeof(std::queue<Buffer*>)]; 
...
struct WeakLLSC{
    ...
    volatile char padding[64-sizeof(std::atomic<Buffer*>)];
    ...
    void retire(Buffer *old){
        Weak_LLSC_Thread_Data &data = llscData[threadID];
        data.rList.push_back(old);
        if(data.rList.size() == 2*MAX_THREADS){ //If we are out of buffers to use, free ones that are in use...
            ...
            for(auto it = data.rList.begin();it != data.rList.end();++it){
                Buffer *buf = *it;
                buf->pid = -1;
