...
do
	...
	do
		...
		for t in {1..5}
		do
			taskset -c "$threads" ./dynamicSetTest -k 20 $s -n $numProcs --output results/evenDistHT.csv -O 1 1 1 1 > errors.txt
		done
	done
	for numProcs in {1,2,3,4,5,6,8,16,32,64,128,256}
	do
		...
		for t in {1..5}
		do
			taskset -c 0-$((numProcs-1)) ./dynamicSetTest -k 20 $s -n $numProcs --output results/insertHeavy.csv -O 4 1 2 2 2> errors.txt
		done
		#Remove heavy 1 4 2 2 
		for t in {1..5}
		do
			taskset -c 0-$((numProcs-1)) ./dynamicSetTest -k 20 $s -n $numProcs --output results/removeHeavy.csv -O 1 4 2 2 2> errors.txt
		done
		#Query heavy 1 1 4 4
		for t in {1..5}
		do
		...
		for t in {1..5}
		do
			taskset -c 0-$((numProcs-1)) ./dynamicSetTest -k 20 $s -n $numProcs --output results/searchHeavy.csv -O 1 1 8 0 2> errors.txt
		done
		#Pred heavy 1 1 0 8 
		for t in {1..5}
		do
			taskset -c 0-$((numProcs-1)) ./dynamicSetTest -k 20 $s -n $numProcs --output results/predHeavy.csv -O 1 1 0 8 2> errors.txt 
		done 
